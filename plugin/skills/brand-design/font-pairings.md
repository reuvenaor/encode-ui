...
| Fraunces + Inter (+ Newsreader body) | luxury, editorial gold-on-ink | Fraunces at display optical size carries the high-contrast headline; Inter carries UI |
| Fraunces + DM Sans | organic-premium, food, expressive | Fraunces's soft wonk headlines over clean geometric UI |
| Newsreader + Inter | editorial body, longform, considered | serif body text, sans UI chrome |
...
| Plus Jakarta Sans + Geist Mono + Newsreader | rounded-professional | ships as the registry's `kino` palette |
| Geist + Geist Mono | Vercel-school minimal | superfamily — zero pairing risk |
| Montserrat + Space Mono + Bitter | geometric display, event/consumer | wide caps love positive tracking |
| Manrope + JetBrains Mono | civic, official, precise | semi-geometric with institutional calm |
...
- Check for a **Reserved Font Name** before you ship a SUBSET of a family. OFL FAQ 2.6
  treats subsetting as a modification, which normally forbids the reserved name, and most
  webfont builds are subsets. The copyright line in the family's licence says whether it
  has one; Lora, Playfair Display and IBM Plex are the common cases. Installing a package
  whole is the easy way to stay clear of the question.
